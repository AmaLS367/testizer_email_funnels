...
- `APP_DRY_RUN`  
  ...
  При `true` скрипт выполняет все операции чтения (выборки), но не отправляет реальных запросов в Brevo и не изменяет данные в таблице `funnel_entries`. Записи в базу данных не выполняются, и вызовы API Brevo не производятся. Скрипт только логирует то, что было бы сделано.  
